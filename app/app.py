...
def fetch_page(): 
    ...
    if 'page' in request.args:
        c.execute("""SELECT ROWID FROM BLOGS WHERE NAME = ?;""",(request.args['page'],))
        ...
        if 'username' in request.form and 'password' in request.form: 
            ...
            if (exists == None): 
                ...
                if len(password) > 7 and len(password) <= 50:
                    c.execute("""INSERT INTO USERS (USERNAME,HASH) VALUES (?,?)""",(request.form['username'],bcrypt.hashpw(password,salt),))
                    db.commit()
                    c.execute("""SELECT USERNAME FROM USERS WHERE USERNAME = ?;""",(request.form['username'],))
                    exists = c.fetchone()
                    db.close()
...
def login():
    if request.method == "POST":
        if 'username' in request.form and 'password' in request.form: 
            db = sqlite3.connect(MAIN_DB)
            c = db.cursor()
            c.execute("""SELECT HASH FROM USERS WHERE USERNAME = ?;""",((request.form['username']).encode('utf-8'),))
            hashed = c.fetchone(); #[0]
            db.close()
            if (hashed == None): 
